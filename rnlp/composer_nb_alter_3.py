...
class ComposerBERT(composer.models.ComposerTransformer):
    def __init__(self, model):
        super().__init__(module=model, config=model.config, model_inputs=set(('input_ids',)), gradient_checkpointing=True)
        #self.module = model

        # Metrics
        #already set by ComposerTransformer.__init__() to composer.metrics.nlp.HFCrossEntropy
    ...
    #ComposerTransformer implements this the same way, with the addition of verifying that the model inputs are present in the batch
    ...
    def loss(self, outputs, batch):
        '''
            outputs: dictionary output from the model
                algorithms can pop the labels from the input in case they modify the loss function
            batch: set of ground truth labels to use to compute the loss against
        '''
        return outputs['loss']

    # ComposerTransformer implements this without including the labels in the output
...
class ComposerGPT(composer.models.ComposerTransformer):
    ignore_index = -100 # composer's default cross entropy leaves this at its default of -100
    def __init__(self, model, tokenizer):
        ...
        self.train_acc = torchmetrics.Accuracy(ignore_index = self.ignore_index)
        self.val_acc = torchmetrics.Accuracy(ignore_index = self.ignore_index)
...
def GPTLanguageAlgorithms(
    min_seq_length = 8,
    max_seq_length = 1024,
    length_warmup_duration = 0.3,
    parallel_cores = 8,
    ...
    return [
        ...
        composer.algorithms.SeqLengthWarmup(
            ...
            duration = length_warmup_duration,
            step_size = parallel_cores,
            truncate = length_truncate,
        )
    ]
...
class ComposerTrainer:
    ...
    def dataset_tokenize(self, dataset, input_col, label_col=None, remove_cols=[], **kwparams):
        kwparams = {'ignore_index': self.model.ignore_index}
